...
from simtk.unit import nanometer, elementary_charge, atmospheres
...
def set_simulation_parameters(pdb_path, platform, properties, args):
    ...
    system = forcefield.createSystem(pdb.topology, nonbondedMethod=NoCutoff, constraints=args.constraints, rigidWater=args.rigidWater)
...
def minimize_and_save(simulation, output_dir, name):
    ...
    relaxed_path = os.path.join(output_dir, name)
    with open(relaxed_path, 'w') as f:
        PDBFile.writeFile(simulation.topology, simulation.context.getState(getPositions=True).getPositions(), f)
    simulation.context.reinitialize(preserveState=True)
    return simulation

def run_simulation(args, fixed_pdb_files):
    platform, properties = set_platform_properties(args)
