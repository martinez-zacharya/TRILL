...
## Set-Up
1. TRILL has only ever been tested on Linux machines, but it might work on Windows with WSL2. You can use TRILL with Google Colab by using the installation instructions from [this notebook](https://colab.research.google.com/drive/1BvJ16EOafg8JmNak19rWiCNgXYLoZChQ?usp=sharing). For regular installations, you need to use pixi. Many thanks to G Anthony Reina for simplifying the installation of TRILL with pixi! If you don't have pixi installed, use this command
```shell
...
```
2. Once pixi software is set up, run
```shell
...
### 1. Finetune Protein Language Models
  ...
  ```
  trill name_of_run 1 finetune esm2_t12_35M trill/data/query.fasta
  ```
  ...
  ```
  trill name_of_run 1 finetune esm2_t36_3B trill/data/query.fasta --strategy deepspeed_stage_2
  ```
  You can finetune ProtGPT2.
  ```
  trill name_of_run 1 finetune ProtGPT2 trill/data/query.fasta
  ```
  ...
  ```
  trill name_of_run 1 finetune ZymCTRL trill/data/query.fasta --ctrl_tag 1.2.3.4
  ```
### 2. Create embeddings
  ...
  ```
  trill name_of_run 1 embed esm2_t12_35M trill/data/query.fasta --avg --per_AA
  ```  
  ...
  ```
  trill name_of_run 1 embed esm2_t33_650M trill/data/query.fasta --batch_size 2 --finetuned /path/to/models/finetuned_esm2_t30_150M_UR50D.pt --avg
  ```
  ...
  ```
  trill name_of_run 1 embed esm2_t12_35M trill/data/query.fasta --poolparti
  ```
  ...
  ```
  trill name_of_run 1 embed SMI-TED trill/data/query.smiles --avg
  ```
### 3. Distributed Training/Inference
  ...
  srun trill name_of_run 4 finetune esm2_t33_650M trill/data/query.fasta --nodes 4 --strategy deepspeed_stage_3_offload
  ```
...
### 4. Language Model Protein Generation
  ...
  ```
  trill name_of_run 1 lang_gen ProtGPT2 --num_return_sequences 5
  ```
  ...
  ```
  trill name_of_run 1 lang_gen ProtGPT2 --finetuned /path/to/FineTune_ProtGPT2_100.pt
  ```
  ...
  ```
  trill name_of_run 0 lang_gen ESM2 --finetuned /path/to/finetuned_model.pt --esm2_arch esm2_t30_150M_UR50D --num_return_sequences 5
  ```
  ...
  ```
  trill name_of_run 1 lang_gen ZymCTRL --num_return_sequences 5 --ctrl_tag 3.1.1.101
  ```
### 5. Inverse Folding Protein Generation
  ...
  ```
  trill name_of_run 1 inv_fold_gen ESM-IF1 trill/data/4ih9.pdb --num_return_sequences 3
  ```
  ...
  ```
  trill name_of_run 1 inv_fold_gen LigandMPNN 7t0r --max_length 1000 --num_return_sequences 5
  ```
  ...
  ```
  trill name_of_run 1 inv_fold_gen ProstT5 trill/data/4ih9.pdb --max_length 1000 --num_return_sequences 5 --top_p 0.9
  ```
### 6. Diffusion based Protein Generation
  ...
  ```
  trill name_of_run 1 diff_gen RFDiffusion --query trill/data/insulin_target.pdb --contigs 'A1-150/0 70-100' --hotspots 'A59,A83,A91' --num_return_sequences 3
  ```
  ...
  ```
  trill name_of_run 1 diff_gen Genie2 --query path/to/example.pdb --contigs 300-350 --motifs 'A-15-100 B-100-150'
  ```
### 7. Predicting protein 3D structures
  ...
  ```
  trill name_of_run 1 fold ESMFold trill/data/query.fasta
  ```
  ...
  ```
  trill name_of_run 1 fold Chai-1 trill/data/chai_example_input.fasta
  ```
  ...
  ```
  trill name_of_run 1 fold Chai-1 trill/data/chai_example_input.fasta --msa
  ```
  ...
  ```
  trill name_of_run 1 fold ProstT5 trill/data/query.fasta
  ```
### 8. Docking
  DiffDock: Uses deep-learning to dock small-molecules to proteins. The output is ranked poses of the ligand.
  ```
  trill name_of_run 1 dock DiffDock trill/data/4ih9.pdb trill/data/NAG_ideal.sdf
  ```
  Lightdock: Dock proteins to proteins using physics-based glowworm swarm optimization 
  ```
  trill name_of_run 1 dock LightDock trill/data/4ih9.pdb trill/data/peptide.pdb
  ```
  ...
  ```
  trill name_of_run 1 dock Vina trill/data/4ih9.pdb ligand_1.sdf ligand_2.sdf --blind
  ```
  ...
  ```
  trill name_of_run 1 dock Vina trill/data/4ih9.pdb ligand_1.sdf --min_alpha_spheres 25
  ```
...
### 9. Visualize your embeddings
  ...
  ```
  trill name_of_run 1 visualize /path/to/embeddings.csv
  ```
### 10. Perform molecular simulations.
  ...
  ```
  trill name_of_run 1 simulate /path/to/protein.pdb --just_relax
  ```
  ...
  ```
  trill name_of_run 1 simulate /path/to/protein.pdb --ligand /path/to/ligand.sdf
  ```
  ...
  ```
  trill name_of_run 1 simulate /path/to/protein.pdb --ligand /path/to/ligand.sdf --solvent ‘amber14/tip3p.xml’
  ```
...
### 11. Leverage pretrained classifiers or train your own custom ones.
  ...
  ```
  trill name_of_run 1 classify EpHod trill/data/query.fasta
  ```
  ...
  ```
  trill name_of_run 0 utils prepare_class_key --fasta_paths_txt my_classes.txt
  ```
  ...
  ```
  trill name_of_run 1 classify XGBoost train_master.fasta --train_split .8 --key my_key.csv 
  ```
  ...
  ```
  trill name_of_run 1 classify XGBoost test_master.fasta --preTrained my_xgboost.json
  ```
  ...
  ```
  trill name_of_run 1 classify XGBoost train_test.fasta --train_split .8 --key my_key.csv 
  ```
  ...
  ```
  trill name_of_run 1 classify ESM2+MLP train.fasta --key key.csv --epochs 1 --train_split 0.9
  ```
  ...
  ```
  trill name_of_run 1 classify iForest train.fasta --emb_model esm2_t30_150M
  trill name_of_run 1 classify iForest test.fasta --emb_model esm2_t30_150M --preTrained my_iforest.skops
  ```
### 12. Use end-to-end workflows.
  ...
  ```
  trill name_of_run 1 workflow foldtune example.fasta
  ```
  ...
  ```
  trill name_of_run 1 workflow foldtune example.fasta --fast_folding 
  ```
## Misc. Tips
...
- **Safetensors Support**: TRILL now supports safer model loading with safetensors format. When loading PyTorch models (.pt files), TRILL will automatically:
  ...
  - You can convert existing .pt files to safetensors format using the utils command:
    ```
    trill name_of_run 0 utils convert_to_safetensors --pt_file model.pt
    ```
...
- If you are using TRILL on Google Colab, you need to start your commands with an "!".
  ```
  !trill name_of_run 1 embed esm2_t12_35M trill/data/query.fasta
  ```
